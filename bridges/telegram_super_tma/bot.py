...
AI_MODERATION_URL = config.get("rechain", {}).get(
    "ai_moderation_url", "https://rechain.example.com/ai/moderate"
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await analytics_log_event(
        "command", {"command": "start", "user": update.effective_user.id}
    )
...
async def bridge_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Bridge is running. Telegram and Matrix connections are active."
    )
    await analytics_log_event(
        "command", {"command": "bridge_status", "user": update.effective_user.id}
    )
...
async def bridge_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await analytics_log_event(
        "command", {"command": "bridge_help", "user": update.effective_user.id}
    )
...
async def bridge_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tg_chat_id in dynamic_mappings:
        ...
        await analytics_log_event(
            "command",
            {"command": "bridge_remove", "user": user_id, "tg_chat_id": tg_chat_id},
        )
    ...
    if chat_id in dynamic_mappings:
        ...
        if update.message.text:
            ...
            if is_clean:
                ...
                print(
                    f"[Telegram->Matrix] {chat_id} -> {room_id}: {update.message.text}"
                )
                await analytics_log_event(
                    "tg_to_matrix",
                    {"from": chat_id, "to": room_id, "text": update.message.text},
                )
                log_event(
                    f"[Telegram->Matrix] {chat_id} -> {room_id}: {update.message.text}"
                )
                ...
                await analytics_log_event(
                    "tg_to_matrix_blocked",
                    {"from": chat_id, "to": room_id, "text": update.message.text},
                )
                log_event(
                    f"[AI Moderation Blocked] {chat_id} -> {room_id}: {update.message.text}"
                )
        # Handle photo
            ...
            await analytics_log_event(
                "tg_to_matrix", {"from": chat_id, "to": room_id, "photo": True}
            )
        ...
        # Handle document
            ...
            await file.download_to_drive(file_path)
            await matrix_bridge.send_media(
                room_id, file_path, mime_type=update.message.document.mime_type
            )
            os.remove(file_path)
            print(f"[Telegram->Matrix] {chat_id} -> {room_id}: [Document]")
            await analytics_log_event(
                "tg_to_matrix", {"from": chat_id, "to": room_id, "document": True}
            )
...
async def send_to_telegram(chat_id, text, file_path=None):
    try:
        if file_path and os.path.exists(file_path):
            ...
            await analytics_log_event(
                "matrix_to_tg", {"to": chat_id, "file": file_path}
            )
        ...
        await analytics_log_event(
            "matrix_to_tg_error", {"to": chat_id, "error": str(e)}
        )
...
async def main():
    ...
    load_plugins(app)
    await asyncio.gather(app.run_polling(), matrix_bridge.run())
