...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            "/data/bridge.log", maxBytes=100 * 1024 * 1024, backupCount=5
        ),
    ],
)
...
class BridgeConfig:
    ...
    def load(cls, config_path: str = "config.yaml") -> "BridgeConfig":
        ...
        return cls(
            ...
            message_processing_workers=bridge_config.get(
...
class MatrixBridge:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.rate_limiter = RateLimiter(
            self.config.rate_limit_rate, self.config.rate_limit_burst
        )
        self.circuit_breaker = CircuitBreaker(
            self.config.circuit_breaker_failures, self.config.circuit_breaker_timeout
        )
                        ...
                        async with session.get(http_url) as resp:
                            if resp.status == 200:
                                ...
                                for (
                                    tg_chat,
                                    mx_room,
                                ) in self.config.chat_room_mapping.items():
                                    if (
                                        mx_room == room.room_id
                                        and self.telegram_callback
                                    ):
    ...
    async def _forward_to_telegram(
        self, chat_id: str, message: Optional[str], file_path: Optional[str] = None
    ):
    ...
    async def send_media(
        self, room_id: str, file_path: str, mime_type: Optional[str] = None
    ):
