...
RECHAIN_ANALYTICS_URL = config.get("rechain", {}).get(
    "analytics_url", "https://rechain.example.com/analytics"
)
RECHAIN_ANALYTICS_KEY = config.get("rechain", {}).get("analytics_api_key", "demo-key")


async def log_event(event_type, data):
    payload = {"event_type": event_type, "data": data, "api_key": RECHAIN_ANALYTICS_KEY}
